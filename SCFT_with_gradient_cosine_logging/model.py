...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=3, spec_norm=False, LR=0.2, feature_list=True):
...
class Generator(nn.Module):
    ...
    def __init__(self, spec_norm=False, LR=0.2):
        ...
        self.encoder_reference = Encoder(in_channels=3, spec_norm=spec_norm, LR=LR, feature_list=False)
    ...
    def forward(self, reference, sketch):
        # 特征提取
        v_r = self.encoder_reference(reference)
        v_s, feature_list = self.encoder_sketch(sketch)

        # 匹配融合，调整feature
        v_c = self.scft(v_s, v_r)
    ...
    def predict(self, reference, sketch):
        ...
        # 匹配融合，调整feature
        v_c, before = self.scft.predict(v_s, v_r)

        rv_c = self.res_model(v_c)
        concat = torch.cat([rv_c, v_c], dim=1)
        image = self.decoder(concat, feature_list)
        return image, before.reshape(b, c, h*w).permute(0, 2, 1), v_c.reshape(b, c, h*w).permute(0, 2, 1)

    def gradient_cosine(self, reference, sketch):
        # 特征提取
        v_r = self.encoder_reference(reference)
        v_s, feature_list = self.encoder_sketch(sketch)

        # 匹配融合，调整feature
        v_c, Z_grad, Q_grad, K_grad, V_grad = self.scft.gradient_cosine(v_s, v_r)

        rv_c = self.res_model(v_c)
        concat = torch.cat([rv_c, v_c], dim=1)
        image = self.decoder(concat, feature_list)
        return image, Z_grad, Q_grad, K_grad, V_grad
